...
docker buildx build --push --builder="$BUILDER" --platform=linux/amd64,linux/arm64 --build-arg TAG=$NEXUS_VERSION --tag="$IMAGE_REPO/nexus3:$NEXUS_VERSION" -f "$NEXUS_DOCKER_FILE" .

docker buildx build --push --builder="$BUILDER" --platform=linux/amd64,linux/arm64 --build-arg TAG="${NEXUS_CONFIG_VERSION:1}" --tag="$IMAGE_REPO/nexus3-config:${NEXUS_CONFIG_VERSION:1}" -f "$NEXUS_CONFIG_DOCKER_FILE" .
